...
static int partition_block_read(struct block_device *dev, uint32_t block_num, uint16_t *buffer)
{
    if (!dev || !buffer)
        return -1;
...
static int partition_block_write(struct block_device *dev, uint32_t block_num, uint16_t *buffer)
{
    if (!dev || !buffer)
        return -1;
