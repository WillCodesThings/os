...
static int ata_read_block(block_device_t *dev, uint32_t block_num, uint8_t *buffer)
{
    if (!dev || !buffer)
        return -1;
...
static int ata_write_block(block_device_t *dev, uint32_t block_num, uint8_t *buffer)
{
    if (!dev || !buffer)
        return -1;
