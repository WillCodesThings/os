...
// Initialize the heap
void heap_init(void) {
    LOG_INFO("MEM", "Initializing kernel heap...");
    ...
    LOG_INFO("MEM", "Heap initialized at %p (%u KB)", (uintptr_t)HEAP_START, HEAP_SIZE / 1024);
}
...
void *kmalloc(size_t size) {
    ...
    if (!block) {
        LOG_ERROR("MEM", "kmalloc: out of memory!");
        return NULL;
    }
...
void kfree(void *ptr) {
    ...
    if (block->magic != HEAP_MAGIC) {
        LOG_ERROR("MEM", "kfree: invalid pointer or corrupted heap at %p", (uintptr_t)ptr);
        return;
    }

    LOG_TRACE("MEM", "kfree: ptr=%p size=%u", (uintptr_t)ptr, (uint32_t)block->size);

    if (!block->used) {
        LOG_ERROR("MEM", "kfree: double free detected at %p", (uintptr_t)ptr);
        return;
    }
